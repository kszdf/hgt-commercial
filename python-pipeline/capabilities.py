...
# ---------------------------------------------------------------------------
# 未开发能力清单（2026-09-18 定：暂不开发，已从 CAPABILITIES 彻底移除）
#   背景：平台定位收窄为「线上自媒体内容获客内容生产」，运营支线（CRM/1v1/AI客服/
#   矩阵分发/看板）不直接产出内容，张老师拍板暂时不开发。
#   本清单仅供 chat_orchestrator 的"能力诚实层"使用：用户问到时如实说"还没接上、
#   做不到"，并给现有能力的替代方案，绝不瞎答应。
#   8500 的 /crm /booking /reception-config /matrix-config /stats 端点保留不动。
# ---------------------------------------------------------------------------
