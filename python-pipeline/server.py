...
def ai_rewrite(text, mode, focus=None, target_duration=None, preserve=None,
    ...
    # 叙事化铁律(2026-08-27 用户定调): 稿子必须是"讲故事"不是"念文件"——
    # 书面腔在源头就锁死自然度, TTS 只能优化韵律救不回正式文; 独白/对话都要叙事化
    NARRATIVE_RULE = (
        "【叙事化改写铁律】\n"
        "- 把稿子改写成'讲故事'的自然口播，而不是念文件：像给老板讲一件真实发生的事，有画面、有情绪；\n"
        "- 结构按起承转合：起(抛出一个老板熟悉的场景或问题)→承(展开讲清楚)→转(风险/关键转折，制造一点紧张感)"
        "→合(给结论和明确的行动建议)；\n"
        "- 允许用第一人称叙事增加个人色彩('我见过''我处理过''有老板问过我')，口吻真实可信、不吹牛；\n"
        "- 适当加语气词('啊''呢''吧''嘛'，每句至多一两个)和反问/感叹，让语调有起承转合、高低起伏；\n"
        "- 严禁书面腔连接词：'综上所述''此外''值得注意的是''换言之''首先其次最后'等；改用口语连接"
        "('说白了''关键在哪''这里要提醒你''说白了就是这么回事')；\n"
