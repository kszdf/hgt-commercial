...
def ai_topic(industry, keywords, count, platform=None, hotness=None, hook=None, form=None):
    ...
    prompt = (
        ...
        # ★2026-09-19 真机踩坑：主题「滞纳金改成迟纳金」被拆成 8 个"合同写迟纳金 vs 税局收滞纳金"的角度，
        #   全跑偏。根因：模型被上面"两个概念不得互替"这条一引，就往"用词辨析"跑；
        #   而老板说的是立法更名。立法动态与用词辨析是两个题材，必须明确优先级。
        "- ★「A改成B / A改名B / A修订 / A要变了」这类表述，一律按**立法与政策修订动态**出题："
        "先点修订背景与进度（哪部法、哪次征求意见稿或常务会议、当前是否已审议通过、是否已施行），"
        "再讲改了什么、对老板的实际影响；\n"
        "  严禁把它拆成「合同里写的A与税务局收的B是两个词」这类用词辨析角度——那是完全不同的另一个题材。"
        "修订尚未通过/施行的，选题角度里必须体现这一时间状态，不许按已生效新法讲；\n"
        + (f"- 维度约束（必须满足）：\n{dim_block}\n" if dim_block else "")
...
# 收口动作平台合规硬洗（2026-09-19）：营销号那套"评论区扣关键词/私信我领资料"在抖音
# 命中《违规营销：诱导互动》两条要件——以福利为条件 + 无意义暗语评论；视频号另有
# "引导私下交易"细则。prompt 拦不住时代码兜底，命中即整句删除，再补一句合规追问收口。
_CLOSER_VIOLATIONS = (
    r"评论区扣", r"扣\s*[1１]\s*[，,]", r"私信我", r"私信(领|拿|发|回复|说)", r"后台(留言|私信)",
    r"(加|＋)\s*(我)?\s*(微信|微|V|vx|VX)", r"微信同号", r"扫码",     r"免费领", r"(领|发)你?(资料|方案|清单)", r"(资料|清单|白皮书|手册)[^。！？]{0,6}领",
    r"666", r"扣\s*[「『\"']", r"关注我", r"点赞收藏", r"记得(关注|点赞|收藏)", r"转发(给|到)",
    r"主页(有|找)\S{0,6}", r"(想要|报名)\s*[，,]?\s*(扣|评论)",
)
# 违规收口被削掉后的合规兜底（按承接档位给一句真实追问，不是暗语词）
_CLOSER_SAFE_TAIL = {
    "L1": "你要是拿不准自己够不够线，把情况说在评论区，我挑典型的讲。",
    "L2": "各家情形不一样，你把你的情况说在评论区，我帮你判断落在哪一档。",
    "L3": "真踩进去的，说说是哪一年、什么情形，我按专案的思路给你拆。",
    "L4": "今年这些节点有没有人替你盯，自己心里先打个分。",
    "REG": "你现在卡在哪一步，评论区说一下，我按先后顺序给你排。",
}


def _guard_closer(text, funnel=None):
    """删掉违规的收口/引流句；若末尾被削，补一句合规追问，保证结尾仍有收口动作。"""
    if not text:
        return text
    ...
    def bad(s):
        return any(re.search(pat, s) for pat in _CLOSER_VIOLATIONS)

    kept = [p for p in parts if not bad(p)]
    if len(kept) == len(parts):
        return text
    # 整句都被判违规时，退一步按逗号再切，保住前半句的条件铺垫（如"个体户年过120万的，"）
    if not kept:
        for p in parts:
            ok = [s for s in re.split(r"(?<=[，,；;])", p) if s.strip() and not bad(s)]
            if ok:
                seg = "".join(ok).rstrip("，,；;")
                ...
                kept.append(seg)
    out = "".join(kept)
    tail = _CLOSER_SAFE_TAIL.get((funnel or "").upper())
    last_was_bad = bad(parts[-1])
    if not out.strip():
        return tail or text
    if tail and last_was_bad and tail not in out:
        out = out.rstrip()
        if out[-1] not in "。！？!?":
            out += "。"
        out += tail
...
def ai_rewrite(text, mode, focus=None, target_duration=None, preserve=None,
    ...
    PERSONA_RULE = (
        ...
        "- 单人口播一律用第一人称自称（'我''我看''我的判断'），**不准用第三人称自称**"
        "（不写'张老师判断''老张认为''我认为的话'这类把本人当旁人的说法）；\n"
    ...
    SCRIPT_STRUCTURE = (
        ...
        "②冲击式反问（'这笔钱以为是省下了，其实埋的是雷'）、③场景写实（直接给一个老板熟悉的现场，"
        "如'税务事项通知书寄到厂里那天，会计先把三年的账翻了出来'）、④政策直给掀后果"
        "（'征求意见稿改了一个字，欠税的成本可能就没了天花板'）。\n"
        "   开口要落到事实、政策或冲突本身；禁止'有老板问我''客户问我''有人问我'等虚构问答式开场，"
        "也不要用'今天跟大家聊一聊''你知道吗'这类套话。\n"
        ...
        "③【结尾·收口动作】：必须按下方《结尾·收口动作铁律》执行——"
        "落到老板身上的过渡 → 一句后果/边界判断 → 最后一个当场可做的自查动作（动作放最后一句）。\n"
        "   动作必须是正文没说过的'下一步'，不许把正文原话复读一遍当结尾；交给老板自己判断，不推销。\n"
    )
    # 结尾·收口动作（2026-09-19 立）：结尾不是结束语，是这条视频唯一收钱的地方。
    # 内核取自短视频营销共识（让观众承认自己有问题、条件越具体筛得越准、让观众自查说服自己），
    # 但执行形式必须按平台规则改造：抖音《违规营销：诱导互动》细则（2025-08-15 生效）判定
    # "以福利/利益为条件 + 无意义暗语评论"即违规，故一律禁"扣关键词/私信我领资料"式收口。
    CLOSER_RULES = (
        "【结尾·收口动作铁律】结尾不是结束语，是这条视频唯一能收钱的地方。"
        "最后一句必须是老板'当下就能做的一个动作'，而且这个动作要能筛出他是不是你的客户。\n"
        "- 收口三步：先一句过渡落到老板自己身上 → 再一句后果或边界判断 → 最后一句给动作（动作放最后）。\n"
        "- 动作句里要请他把核出来的结果说出来：说个数、说个年份、说个情形（在评论区说就行），"
        "这一步才是收口——他一开口，你就知道他是不是客户。说情况可以，让他扣一个暗语词不行。\n"
        "- 动作必须具体到他当场能做：让他去核一个数、翻一张表、对一个日期/年限/金额，"
        "而不是'要注意风险''提前规划'这类空话。\n"
        "- 条件越具体筛得越准：动作里带上可自查的门槛（年限、金额、身份、是否已经发生），"
        "不符合的人自然划走，留下的才是会掏钱的人。\n"
        "- 让老板自己说服自己：设计成他照着做完会心里一紧的自查动作，我只需给出判断标准，不替他下结论。\n"
        "★平台合规红线（照抄营销号那套'扣关键词/私信我领资料'会被限流，一律不许出现）：\n"
        "  · 禁要求观众发暗语词、'666''想要''报名'等与内容无关的无意义评论；\n"
        "  · 禁以福利换动作：不许'扣XX我发你资料''私信领取''评论区扣1'——以利益为条件的互动即违规；\n"
        "  · 禁留联系方式与站外引流（微信/电话/二维码/加群一律不行）；\n"
        "  · 禁'关注我''点赞收藏''转发给需要的朋友'这类求互动求关注的话（既违规又掉价）。\n"
        "  合规的收口是请他说出自己的情况、自己的数、自己的疑问——这是有信息量的真实表达，"
        "平台算正常互动，你还能一眼看出他是不是客户。\n"
    ...
    FUNNEL_HINTS = {
        "L1": ("本条获客锚点：**L1 税风险检测（钩子款）**——钩子和解决思路要指向『先做个风险检测，把账上的疑点列出来』。"
               "收口动作：让他当场核一个数/翻一张表（自查式），并请他说出自己核出来的情况——"
               "用'金额/年限/笔数'当门槛，够线的自然浮出来。"),
        "L2": ("本条获客锚点：**L2 轻咨询**——钩子和解决思路要指向『这个具体问题可以单独拎出来算一笔账、给个处理方案』。"
               "收口动作：身份对号入座——把'行业+规模+具体情形'三个条件叠上去，"
               "请符合的人说出自己的情形，不符合的人自动划走。"),
        "L3": ("本条获客锚点：**L3 专案讲解（历史遗留/稽查应对，高客单）**——钩子和解决思路要指向"
               "『已经形成的历史问题需要按专案处理』。收口动作：让他按年限和处境自查"
               "（挂了多少年、有没有被约谈/调账/补申报过），踩中的人请他说说是哪一年、什么情形——"
               "高客单靠'说清楚自己的坑'来识别，不靠福利。"),
        "L4": ("本条获客锚点：**L4 年度财税顾问**——钩子和解决思路要指向『常态化合规与年度健康度，需要有人长期盯』。"
               "收口动作：让他想一想今年这些节点有没有人替他盯（汇算清缴、政策到期、股权变动、用工方式变化）；"
               "**不许报价、不许承诺省多少**，只说代价——真到那一步花的就不只这点顾问费。"),
        "REG": ("本条获客锚点：**注册公司引流款**——钩子和解决思路要指向『开业起步阶段的登记、银行、税务、社保节点』。"
                "收口动作：清单式——让他对着自己开业的时间点打勾（执照、刻章、银行、税务报到、社保、记账），"
                "没走到那一步的请他说出现在卡在哪一步。"),
    }
    ...
    NARRATIVE_RULE = (
        ...
        "  ④专业术语必须用规范名称：原稿/需求里明显的错字术语（同音字/形近字）一律纠正后书写"
        "（如'迟滞金'→'迟纳金'、'增植税'→'增值税'、'两套帐'→'两套账'），不要照抄错字；"
        "但规范术语本身不得改写或互替（'滞纳金'与'迟纳金'是现行法与征求意见稿两个不同概念，不得互改），"
        "必要时加一句通俗解释帮老板听懂；\n"
    ...
    cleaned = _guard_closer(cleaned, funnel)
