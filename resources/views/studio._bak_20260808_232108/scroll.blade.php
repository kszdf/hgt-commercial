    ...
    if (params.get('from') === 'rewrite') {
        ...
        if (cleaned) {
            ...
            hint.innerHTML = '<div class="flex items-center justify-between"><span>已从「' + srcInfo.label + '」带入清洗稿。' +
                ...
                '</span></div>' +
                '<a href="' + srcInfo.back + '" class="font-medium underline hover:text-brand-900 text-xs">← 返回二创</a>';
...
function setMode(m) {
    ...
    // 声线选择：数字人=单一声线下拉，滚动字幕=男/女双下拉
    document.getElementById('singleVoiceWrap').classList.toggle('hidden', m !== 'avatar');
    document.getElementById('dualVoiceWrap').classList.toggle('hidden', m !== 'avatar');
    if (m === 'scroll') setVoiceForm(voiceForm);
...
document.querySelectorAll('.voice-form-btn').forEach(b => {
    b.addEventListener('click', () => setVoiceForm(b.dataset.form));
});
